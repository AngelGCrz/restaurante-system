<x-layouts.app>
    <div class="p-4">
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Reporte de Ventas</h1>
            <form method="GET" class="flex items-center gap-2">
                <input type="date" name="start" value="{{ $startDate->format('Y-m-d') }}" class="rounded border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 px-2 py-1 text-sm">
                <input type="date" name="end" value="{{ $endDate->format('Y-m-d') }}" class="rounded border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 px-2 py-1 text-sm">
                <select name="status" class="rounded border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 px-2 py-1 text-sm">
                ...
                <select name="user_id" class="rounded border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 px-2 py-1 text-sm">
        ...
        <div class="rounded-xl border bg-white dark:bg-zinc-900 dark:border-zinc-700 p-4">
            ...
            <table class="w-full text-left text-zinc-900 dark:text-zinc-100">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="4" class="py-4 text-sm text-zinc-500 dark:text-zinc-400">No hay datos en el rango seleccionado.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
